<?php
	...
	$nameMateriaUpdate = $_POST('materiaUpdate'); // Asignamos valor a la variable $nameMateriaUpdate.

	if($nameMateria!="" && $nameMateriaUpdate!=""){ // Verificamos si algunas de estas 2 variables esta con datos vacíos;
		if($query = $conexion->prepare("SELECT count(*) as existe,id_materia  from materias where nombre_mat=?")){ // Preparamos la query.
			...
			if ($existe>0) { // Verificamos si existe la materia ingresada desde el formulario.
				if($query = $conexion->prepare("UPDATE materias SET nombre_mat=? where id_materia=?")){ // Preparamos la query para su modificacion.
					$query->bind_param("sd",$nameMateriaUpdate,$idMateria); // Especificamos que tipo de dato son nuestra variables que utiliza la query UPDATE.
					$query->execute(); // 
					$query->close();
